...
description: ねこプロフィール の columns.content 向けに、元ネタ本文だけを入力として既存記事準拠の HTML 記事本文を生成する。
---

# cat-columns-blog-writer

ねこプロフィール の `columns.content` に保存する HTML 記事本文を生成するためのスキル。
...
<div class="mx-auto py-4 bg-white rounded-lg">
  ...
  <div class="mt-6 bg-gray-100 p-5 rounded-lg shadow-sm border border-gray-200">
    ...
    <p class="text-gray-700 leading-relaxed mb-4">
      ねこプロフィールでは、あなたの愛猫専用のプロフィールページを簡単に作成できます。かわいい写真や日常の記録をまとめて、猫仲間との交流も楽しみましょう！
    </p>
    <a
...
- 本文段落は `p.text-gray-700.leading-relaxed` を基本とし、セクションの最後の段落には `mb-6`、それ以外には `mb-4` を付与する
- 箇条書きは `ul.list-disc.pl-6.text-gray-700.mb-6.space-y-3`
- 箇条書きの各項目 `li` は、`<span class="font-semibold">項目名</span>：説明` の形式を基本とする
...
- 見出し: `h3.text-xl.font-semibold.text-gray-700.mb-3` で `愛猫のプロフィールページを作りませんか？🐾`
- 本文: `ねこプロフィールでは、あなたの愛猫専用のプロフィールページを簡単に作成できます。かわいい写真や日常の記録をまとめて、猫仲間との交流も楽しみましょう！`
- ボタン: `ねこプロフィールでプロフィールを作成する`
